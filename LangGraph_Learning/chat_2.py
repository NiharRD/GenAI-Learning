...
class State(TypedDict): ## this time its not like a list , but a single state like useState in react .
    user_query: str
    llm_output: Optional[str]
    is_good: Optional[bool]

def chatbot(state: State): # in LangGraph functions , expected to retrun a state or node actually  .
...
def evalaute_response(state: State) -> Literal["chatbot_gemini", "endnode"]:
    ...
    user_bool_input = input("Is the response good? (y/n): ")
    state["is_good"] = user_bool_input == "y"
    if   state["is_good"]:
        return "endnode"
